...
fn main() {
    ...
    println!("{:>7}  {:>10}  {:>12}  {:>12}  {:>10}", "time(s)", "total_recv",
             "rate(pkt/s)", "avg_dec(µs)", "errors");
    ...
    loop {
        ...
        match ind_rx.recv_timeout(timeout) {
            Ok(ind) => {
                ...
                match coder.decode(&ind.data) {
                    Ok(_)  => {}
                    Err(e) => { total_errors += 1; eprintln!("[RX] Decode error: {e}"); }
                }
        ...
        if win_elapsed >= Duration::from_secs(1) {
            ...
            let avg_dec = if total_recv > 0 { total_dec_us / total_recv as u128 } else { 0 };
            println!("{:>7.1}  {:>10}  {:>12.1}  {:>12}  {:>10}",
                bench_start.elapsed().as_secs_f64(), total_recv, pps, avg_dec, total_errors);
    ...
    let avg_decode = if total_recv > 0 { total_dec_us / total_recv as u128 } else { 0 };
...
fn random_mac() -> [u8; 6] {
    ...
    [0x02, (s >> 24) as u8, (s >> 16) as u8, (s >> 8) as u8, s as u8, 0xCC]
}
