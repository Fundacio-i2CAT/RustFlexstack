...
use rustflexstack::facilities::ca_basic_service::cam_coder::{
    cam_header, generation_delta_time_now as cam_gdt,
...
use rustflexstack::facilities::decentralized_environmental_notification_service::{
    denm_coder::AccidentSubCauseCode,
    DecentralizedEnvironmentalNotificationService, DENRequest, DenmCoder,
...
use rustflexstack::facilities::vru_awareness_service::{
    vam_coder::{
        generation_delta_time_now as vam_gdt, vam_header,
        AccelerationConfidence as VamAccelConf,
        Altitude as VamAlt, AltitudeConfidence as VamAltConf,
        AltitudeValue as VamAltVal, BasicContainer as VamBasicContainer, Latitude as VamLat,
        Longitude as VamLon,
        LongitudinalAcceleration as VamLongAccel,
        LongitudinalAccelerationValue as VamLongAccelVal,
        PositionConfidenceEllipse as VamPCE,
        ReferencePositionWithConfidence as VamRefPos, SemiAxisLength as VamSAL,
        Speed as VamSpeed, SpeedConfidence as VamSpeedConf, SpeedValue as VamSpeedVal,
        TrafficParticipantType as VamTPT, Vam, VamCoder, VamParameters,
        VruAwareness, VruHighFrequencyContainer,
        Wgs84Angle as VamWgs84Angle, Wgs84AngleConfidence as VamWgs84AnglConf,
        Wgs84AngleValue as VamWgs84AngleVal,
    },
};
...
fn main() {
    ...
    let duration_s = env::args().nth(2).and_then(|s| s.parse::<u64>().ok()).unwrap_or(30);
    ...
    let vd = DenVehicleData { station_id, station_type: 5 };
                ...
                match cam_ind_rx.recv_timeout(timeout) {
                    Ok(ind) => {
                        ...
                        match coder.decode(&ind.data) {
                            Ok(_)  => { cnt.fetch_add(1, Ordering::Relaxed); }
    ...
    println!("{:>6}  {:>8}  {:>8}  {:>8}  {:>8}  {:>8}  {:>8}  {:>8}  {:>8}  {:>8}",
        "t(s)",
        "cam_tx", "cam_rx", "cam_dec_µs",
        "vam_tx", "vam_rx", "vam_dec_µs",
        "den_tx", "den_rx",
        "rx_err",
    );
    ...
    loop {
        ...
        if t >= duration_s as f64 + 1.5 { break; }
        ...
        println!("{:>6.1}  {:>8}  {:>8.1}  {:>10}  {:>8}  {:>8.1}  {:>10}  {:>8}  {:>8.1}  {:>8}",
            t,
            ctx, cam_rx_rate, cam_dec_avg,
            vtx, vam_rx_rate, vam_dec_avg,
            dtx, denm_rx_rate,
            errs,
        );

        if t > duration_s as f64 { break; }
    }
    ...
    let cam_ratio  = if ctx > 0 { crx  as f64 / ctx  as f64 * 100.0 } else { 0.0 };
    let vam_ratio  = if vtx > 0 { vrx  as f64 / vtx  as f64 * 100.0 } else { 0.0 };
    let denm_ratio = if dtx > 0 { drx  as f64 / dtx  as f64 * 100.0 } else { 0.0 };

    println!();
    println!("=== Congestion Benchmark Results ({elapsed:.1} s) ===");
    println!();
    println!("  CAM   TX: {ctx:>8}  RX: {crx:>8}  ratio: {cam_ratio:>6.1}%  avg_dec: {cam_dec_avg} µs");
    println!("  VAM   TX: {vtx:>8}  RX: {vrx:>8}  ratio: {vam_ratio:>6.1}%  avg_dec: {vam_dec_avg} µs");
    ...
    Vam::new(
        ...
        VruAwareness::new(
            ...
            VamParameters::new(
                ...
                hf,
                None, None, None, None, // lf, cluster_info, cluster_op, motion_pred
            ),
        ),
    )
}
...
fn random_mac() -> [u8; 6] {
    ...
    [0x02, (s >> 24) as u8, (s >> 16) as u8, (s >> 8) as u8, s as u8, 0xDD]
}
