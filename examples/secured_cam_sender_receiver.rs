...
fn main() {
    ...
    // ── GPS publisher (random trajectory — mirrors Python _RandomTrajectoryLocationService) ──
    //
    // Updates every 80 ms (below T_CheckCamGen = 100 ms) with a randomly
    // changing heading (±5–15° per step, always exceeding the 4° condition-1
    // threshold) and speed random-walking within [5, 20] m/s.  This sustains
    // 10 Hz CAM generation via the dynamics triggering conditions.
    thread::sleep(Duration::from_millis(100));
    println!("Publishing GPS fixes @ 12.5 Hz (80 ms) with random trajectory — Ctrl+C to stop\n");

    use rand::Rng;
    let mut rng = rand::thread_rng();

    const PERIOD_S: f64 = 0.08; // 80 ms — below T_CheckCamGen
    const EARTH_R: f64 = 6_371_000.0;

    let mut lat: f64 = 41.386931;
    let mut lon: f64 = 2.112104;
    let mut heading: f64 = rng.gen_range(0.0..360.0);
    let mut speed: f64 = 10.0; // ~36 km/h

    loop {
        thread::sleep(Duration::from_millis(80));

        // Heading: random signed change guaranteed to exceed the 4° threshold
        let delta: f64 = rng.gen_range(5.0..15.0) * if rng.gen_bool(0.5) { 1.0 } else { -1.0 };
        heading = (heading + delta).rem_euclid(360.0);

        // Speed: small random walk within [5, 20] m/s
        speed = (speed + rng.gen_range(-0.5..0.5)).clamp(5.0, 20.0);

        // Position update (flat-Earth approximation; step sizes < 2 m)
        let d = speed * PERIOD_S;
        let heading_r = heading * PI / 180.0;
        let lat_r = lat * PI / 180.0;
        lat += (d * heading_r.cos() / EARTH_R) * (180.0 / PI);
        lon += (d * heading_r.sin() / (EARTH_R * lat_r.cos())) * (180.0 / PI);
