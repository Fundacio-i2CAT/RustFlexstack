...
//! 4. Decoding exceptions are caught and logged; the LDM/applications are NOT
//!    updated with corrupt data (Annex B.3.3.1).
//! 5. Optionally stores the decoded CAM in the LDM (if a handle is provided).
//! 6. Forwards the decoded [`Cam`] to the caller via an MPSC channel.
                ...
                match coder.decode(&indication.data) {
                    Ok(cam) => {
                        ...
                        if let Some(ref ldm_handle) = ldm {
                            let ref_pos =
                                &cam.cam.cam_parameters.basic_container.reference_position;
