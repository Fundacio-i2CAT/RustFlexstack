...
use super::denm_coder::{
    denm_header, timestamp_its_now, ActionId, Altitude, AltitudeConfidence,
    AltitudeValue, CauseCodeChoice, CauseCodeV2, DeltaAltitude, DeltaLatitude,
    DeltaLongitude, DeltaReferencePosition, DeltaTimeMilliSecondPositive,
    DeltaTimeSecond, Denm, DenmCoder, DenmPayload, InformationQuality, Latitude,
    Longitude, ManagementContainer, Path, PathPoint, PosConfidenceEllipse,
    ReferencePosition, SemiAxisLength, SequenceNumber, SituationContainer,
    Speed, SpeedConfidence, SpeedValue, StationId, StationType, SubCauseCodeType,
    Traces, TrafficParticipantType, Wgs84Angle, Wgs84AngleConfidence,
    Wgs84AngleValue, LocationContainer, HeadingValue,
};
...
use crate::geonet::service_access_point::{
    Area, CommunicationProfile, CommonNH, GeoBroadcastHST, HeaderSubType,
    HeaderType, PacketTransportType, TrafficClass,
};
...
impl Default for DENRequest {
    fn default() -> Self {
        DENRequest {
            ...
            cause_code:        CauseCodeChoice::accident2(
                super::denm_coder::AccidentSubCauseCode(0),
            ),
...
pub fn build_denm(request: &DENRequest, vd: &VehicleData, seq_nr: u16) -> Denm {
    ...
    let event_pos = ReferencePosition::new(
        Latitude(
            ((request.event_latitude * 1e7).round() as i32)
                .clamp(-900_000_000, 900_000_000),
        ),
        Longitude(
            ((request.event_longitude * 1e7).round() as i32)
                .clamp(-1_800_000_000, 1_800_000_000),
        ),
        ...
        Altitude::new(
            AltitudeValue(
                ((request.event_altitude_m * 100.0).round() as i32)
                    .clamp(-100_000, 800_000),
            ),
...
impl DENMTransmissionManagement {
    pub fn new(
        ...
        DENMTransmissionManagement { btp_handle, coder, vehicle_data }
    }
    ...
    pub fn trigger_denm_sending(&self, request: DENRequest) {
        ...
        thread::spawn(move || {
            ...
            let end_time   = std::time::Instant::now()
                + Duration::from_millis(request.time_period_ms);
